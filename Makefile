...
ARGOCD_PROD_PASSWORD ?= $(shell kubectl --context=production -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" 2>/dev/null | base64 -d)
ARGOCD_DEV_PASSWORD  ?= $(shell kubectl --context=develop -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" 2>/dev/null | base64 -d)
...
start-clusters:
	...
	minikube start -p develop

## 2a. Install ArgoCD on production (prod - manages production + cluster2)
install-argocd-prod:
	kubectl create namespace argocd --dry-run=client -o yaml | kubectl apply --context=production -f -
	kubectl apply --context=production -n argocd --server-side --force-conflicts \
		-f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml

## 2b. Install ArgoCD on develop (dev - manages develop)
install-argocd-dev:
	kubectl create namespace argocd --dry-run=client -o yaml | kubectl apply --context=develop -f -
	kubectl apply --context=develop -n argocd --server-side --force-conflicts \
		-f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml

## 3a. Wait for ArgoCD prod to be ready
wait-argocd-prod:
	kubectl wait --context=production --for=condition=ready pod \
		-l app.kubernetes.io/name=argocd-server -n argocd --timeout=180s

## 3b. Wait for ArgoCD dev to be ready
wait-argocd-dev:
	kubectl wait --context=develop --for=condition=ready pod \
		-l app.kubernetes.io/name=argocd-server -n argocd --timeout=180s

## 4a. Port forward ArgoCD prod UI on :8080 (open a separate terminal)
port-forward-argocd-prod:
	kubectl port-forward --context=production svc/argocd-server -n argocd 8080:443

## 4b. Port forward ArgoCD dev UI on :8081 (open a separate terminal)
port-forward-argocd-dev:
	kubectl port-forward --context=develop svc/argocd-server -n argocd 8081:443

## 5a. Print initial admin password (prod)
get-password-prod:
	kubectl --context=production -n argocd get secret argocd-initial-admin-secret \
		-o jsonpath="{.data.password}" | base64 -d && echo

## 5b. Print initial admin password (dev)
get-password-dev:
	kubectl --context=develop -n argocd get secret argocd-initial-admin-secret \
		-o jsonpath="{.data.password}" | base64 -d && echo
